...
export async function fetchTenantById(
  ...
  try {
    ...
    const tenantDocSnap: DocumentSnapshot<DocumentData> = await getDoc(
      tenantDocRef
    );
...
export async function fetchTenantAndFoodById(
  tenantId: string
): Promise<Tenant | undefined> {
  try {
    const tenantDocRef = doc(db, "tenant", tenantId);
...
export async function GetApprovedTenants() {
  ...
  querySnapshot.forEach((document) => {
    const tenantData = document.data() as Tenant;
    tenantData.tenantID = document.id;
